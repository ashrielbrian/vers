...
impl<const N: usize> Vector<N> {
    ...
    pub fn cosine_similarity(&self, other: &Vector<N>, normalized: bool) -> f32 {
        if !normalized {
            ...
            return self.dot_product(other) / (norm_a * norm_b);
        }

        // returns the cosine distance (cosine distance = 1 - cosine similarity), so range is [0, 2]
        // smaller is closer
        1.0 - self.dot_product(other)
    }

    pub fn cosine_similarity_simd(&self, other: &Vector<N>, _normalized: bool) -> f32 {
        ...
        if chunks_64 > 0 {
            ...
            for i in 0..chunks_64 {
                unsafe {
                    res += (*simd_u_ptr_64.offset(i) * *simd_v_ptr_64.offset(i)).reduce_sum();
                }
            }
        }
        ...
        if chunks_4 > 0 {
            ...
            for i in 0..chunks_4 {
                unsafe {
                    res += (*simd_u_ptr_4.offset(i) * *simd_v_ptr_4.offset(i)).reduce_sum();
                }
            }
        }
        ...
        if final_remaining > 0 {
            for i in processed_4..vec_len {
                unsafe {
                    res += *u_ptr.offset(i) * *v_ptr.offset(i);
                }
            }
        }
